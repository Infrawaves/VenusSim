...
Sys::Sys(
  ...
  logical_sub_topologies["AllReduce"] = new GeneralComplexSubTopology(
      id, physical_dims, all_reduce_implementation_per_dimension);
  logical_sub_topologies["ReduceScatter"] = new GeneralComplexSubTopology(
      id, physical_dims, reduce_scatter_implementation_per_dimension);
  logical_sub_topologies["AllGather"] = new GeneralComplexSubTopology(
      id, physical_dims, all_gather_implementation_per_dimension);
  logical_sub_topologies["AllToAll"] = new GeneralComplexSubTopology(
      id, physical_dims, all_to_all_implementation_per_dimension);
  ...
  workload =
      new Workload(this, workload_configuration, comm_group_configuration, trace_template_mapping_id);
  ...
  this->trace_template_mapping_id = trace_template_mapping_id;
  this->trace_output_mapping_id = trace_output_mapping_id;
...
bool Sys::initialize_sys(string name) {
  ...
  if(j.contains("ignore-bandwidth")){
    if (j["ignore-bandwidth"] != 0) {
      this->ignore_bandwidth = true;
    } else {
      this->ignore_bandwidth = false;
    }
  }
  else{
  ...
  if (j.contains("roofline-enabled")) {
    if (j["roofline-enabled"] != 0) {
      ...
      roofline = new Roofline(local_mem_bw, peak_perf, mfu);
    }
  }
  ...
  this->simplifed_trace = false;
  if (j.contains("simplified-trace")) {
    if (j["simplified-trace"] != 0) {
      this->simplifed_trace = true;
    } else {
      this->simplifed_trace = false;
    }
  }
  
  this->template_trace = false;
  if (j.contains("template-trace")) {
    if (j["template-trace"] != 0) {
      this->template_trace = true;
    } else {
      this->template_trace = false;
    }
  }

  this->log_level = LOG_NONE;
  if (j.contains("log-level")){
    if (j["log-level"] == 0){
      this->log_level = LOG_NONE;
    }
    else if(j["log-level"] == 1){
      this->log_level = LOG_NORMAL;
    }
    else if(j["log-level"] == 2){
      this->log_level = LOG_DEBUG;
    }
  }
  if (j.contains("align-with-nccl")) {
    if(j["align-with-nccl"] == 0){
      this->align_to_nccl = false;
    }else if (j["align-with-nccl"] == 1)
    {
      this->align_to_nccl = true;
    }
    }
  inFile.close();
  return true;
...
LogicalTopology* Sys::get_logical_sub_topology(ComType comm_type) {
  if (comm_type == ComType::All_Reduce)
    return logical_sub_topologies["AllReduce"];
  else if (comm_type == ComType::All_to_All)
    return logical_sub_topologies["AllToAll"];
  else if (comm_type == ComType::Reduce_Scatter)
    return logical_sub_topologies["ReduceScatter"];
  else if (comm_type == ComType::All_Gather)
    return logical_sub_topologies["AllGather"];
  else {
    sys_panic("no known logical topology!");
    return nullptr;
  }
...
DataSet* Sys::generate_all_reduce(
  ...
  if (communicator_group == nullptr) {
    ...
    if(log_level == LOG_DEBUG){
      cout<<"------------------generate_all_reduce with communicator_group"<<endl;
    }
    CollectivePlan* plan =
...
DataSet* Sys::generate_collective(
  ...
  if(this->log_level == LOG_DEBUG)
    std::cout<<"*****************"<<communicator_group->get_id()<<" "<<dataset->my_id<<" "<<(int)collective_type<<" "<<(int)inter_dimension_scheduling<<std::endl;
  ...
  while (size > 0) {
    ...
    if (collective_type != ComType::All_Reduce ||
      ...
      while (dim > 0 &&
        ...
        dim--;
	std::cout<<"hhhhhhhhhh111-"<<std::endl;
      }
    ...
    if (vect.size() > 0) {
      ...
      std::tuple<int,int,bool> nccl_correct_factor = std::make_tuple(communicator_group->group_dim0_gpu_num,communicator_group->group_gpu_num,Sys::align_to_nccl);
      StreamBaseline* newStream =
          new StreamBaseline(this, dataset, stream_id, vect, pri,nccl_correct_factor);
      newStream->current_queue_id = -1;
      insert_into_ready_list(newStream);
      if(this->log_level == LOG_DEBUG)
        std::cout<<"-------------------------***"<<communicator_group->get_id()<<" "<<communicator_group->num_streams<<" "<<stream_id<<" "<<vect.size()<<std::endl;
    } else {
