...
void HardwareResource::occupy(const shared_ptr<SelectETFeederNode> node) {
  // for trace generated by trace-generator, all nodes are cpu-node
  // if (node->is_cpu_op()) {
  ...
  // } else 
  {
    if(node->name().find("dummy") != std::string::npos)
      return;
    if (node->type() == SelectNodeType::COMP_NODE){
...
void HardwareResource::release(const shared_ptr<SelectETFeederNode> node) {
  // for trace generated by trace-generator, all nodes are cpu-node
  // if (node->is_cpu_op()) {
  //   --num_in_flight_cpu_ops;
  //   assert(num_in_flight_cpu_ops == 0);
...
bool HardwareResource::is_available(
    const shared_ptr<SelectETFeederNode> node) const {
  // if (node->is_cpu_op()) {
