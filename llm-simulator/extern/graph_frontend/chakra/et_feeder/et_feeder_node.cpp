...
ETFeederNode::ETFeederNode(std::shared_ptr<ChakraProtoMsg::Node> node) {
  ...
  for (int i = 0; i < node->attr_size(); i++) {
    ...
    if (attr_name == "is_cpu_op") {
      ...
      //cout<<"-----------------------------------------num_ops_ "<<this->name_<<" "<<this->num_ops_<<endl;
    } else if (attr_name == "tensor_size") {
      assign_attr_val(node, i, (void *)(&tensor_size_));
      //cout<<"-----------------------------------------tensor_size_ "<<this->name_<<" "<<this->tensor_size_<<endl;
      ...
      //cout<<"-----------------------------------------comm_size_ "<<this->name_<<" "<<this->comm_size_<<endl;
...
string ETFeederNode::comm_group() {
  return comm_group_;
}
