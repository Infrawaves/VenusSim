...
def initialize_model_parallel(
    ...
    global _GLOBAL_RANK
    _GLOBAL_RANK = rank
    assert npu_rank < world_size, f"npu rank {npu_rank} out of range: {world_size}."
    ...
    for i in range(pipeline_model_parallel_size):
        ...
        for j in range(context_parallel_size * tensor_model_parallel_size):
            ...
            if rank in ranks:
                _DATA_PARALLEL_GROUP = list(ranks)
                _DATA_PARALLEL_GROUP_GLOO = list(ranks)
        ...
        for j in range(tensor_model_parallel_size):
            ...
            if rank in ranks_with_cp:
                _DATA_PARALLEL_GROUP_WITH_CP = list(ranks_with_cp)
                _DATA_PARALLEL_GROUP_WITH_CP_GLOO = list(ranks_with_cp)
    ...
    for i in range(pipeline_model_parallel_size):
        for j in range(data_parallel_size):
            ...
            for k in range(tensor_model_parallel_size):
                ...
                # )
                if rank in ranks:
                    _CONTEXT_PARALLEL_GROUP = list(ranks)
    ...
    for i in range(data_parallel_size * context_parallel_size):
        ...
        # )
        if rank in ranks:
            _MODEL_PARALLEL_GROUP = list(ranks)
    ...
    for i in range(num_tensor_model_parallel_groups):
        ...
        # )
        if rank in ranks:
            _TENSOR_MODEL_PARALLEL_GROUP = list(ranks)
    ...
    for i in range(num_pipeline_model_parallel_groups):
        ...
        # )
        if rank in ranks:
            _PIPELINE_MODEL_PARALLEL_GROUP = list(ranks)
        ...
        if rank in embedding_ranks:
            _EMBEDDING_GROUP = list(embedding_ranks)
        if rank in ranks:
        ...
        if rank in position_embedding_ranks:
            _POSITION_EMBEDDING_GROUP = list(position_embedding_ranks)
        if rank in ranks:
    ...
    for i in range(num_tensor_and_data_groups_with_cp):
        ...
        # )
        if rank in ranks:
            _TENSOR_AND_DATA_PARALLEL_GROUP_WITH_CP = list(ranks)
        ...
        for j in range(context_parallel_size):
            ...
            # )
            if rank in ranks:
                _TENSOR_AND_DATA_PARALLEL_GROUP = list(ranks)
    ...
    for i in range(num_tensor_and_data_groups):
        for j in range(num_expert_groups):
            ...
            # )
            if rank in ranks:
                _TENSOR_AND_EXPERT_PARALLEL_GROUP = list(ranks)
    ...
    for i in range(num_tensor_and_data_groups):
        ...
        for j in range(tensor_and_expert_group_size):
            ...
            if rank in ranks:
                _DATA_MODULO_EXPERT_PARALLEL_GROUP = list(ranks)
                _DATA_MODULO_EXPERT_PARALLEL_GROUP_GLOO = list(ranks)
...
def is_embedding_group_initialized():
    return (_EMBEDDING_GROUP is not None)
...
def is_position_embedding_group_initialized():
    return (_POSITION_EMBEDDING_GROUP is not None)
...
def get_pipeline_model_parallel_world_size():
    ...
    return len(get_pipeline_model_parallel_group())
...
def get_tensor_model_parallel_rank():
    ...
    return get_tensor_model_parallel_group().index(_GLOBAL_RANK)
...
def get_pipeline_model_parallel_rank():
    ...
    return get_pipeline_model_parallel_group().index(_GLOBAL_RANK)
...
def get_data_parallel_world_size(with_context_parallel=False):
    ...
    return len(get_data_parallel_group(with_context_parallel=with_context_parallel))
...
def get_data_parallel_rank(with_context_parallel=False):
    ...
    return get_data_parallel_group(with_context_parallel=with_context_parallel).index(_GLOBAL_RANK)
    if torch.distributed.is_available() and torch.distributed.is_initialized():
...
def get_context_parallel_world_size():
    ...
    return len(get_context_parallel_group())
    if torch.distributed.is_available() and torch.distributed.is_initialized():
...
def get_context_parallel_rank():
    ...
    return get_context_parallel_group().index(_GLOBAL_RANK)
    if torch.distributed.is_available() and torch.distributed.is_initialized():
...
def get_expert_model_parallel_world_size():
    ...
    tensor_and_expert_parallel_world_size = len(get_tensor_and_expert_parallel_group())
...
def get_tensor_and_expert_parallel_world_size():
    ...
    """
    return len(get_tensor_and_expert_parallel_group())
    if torch.distributed.is_available() and torch.distributed.is_initialized():
        tensor_and_expert_parallel_world_size = torch.distributed.get_world_size(
            group=get_tensor_and_expert_parallel_group()
        )
...
def get_expert_model_parallel_rank():
    ...
    tensor_and_expert_parallel_rank = get_tensor_and_expert_parallel_group().index(_GLOBAL_RANK)
...
def get_data_modulo_expert_parallel_rank():
    """Return my rank for the context parallel group."""
    return get_data_modulo_expert_parallel_group().index(_GLOBAL_RANK)
    if torch.distributed.is_available() and torch.distributed.is_initialized():
...
def get_global_rank():
    if _GLOBAL_RANK is None:
        assert False, "_GLOBAL_RANK is None."
    return _GLOBAL_RANK
