...
from generator.megatron_core.optimizer import MixedPrecisionOptimizer
...
PARAM_ALL_GATHER_NUM = None

def set_param_all_gather_num(param_num):
    global PARAM_ALL_GATHER_NUM
    PARAM_ALL_GATHER_NUM = param_num

def get_param_all_gather_num():
    global PARAM_ALL_GATHER_NUM
    return PARAM_ALL_GATHER_NUM
...
class DistributedOptimizer(MixedPrecisionOptimizer):
    ...
    def get_param_allgather_node(
        ...
        if self.skip_param_gather:
            node = TraceNode(
                ...
                # "~unique~" + str(param_ag_unique_id[self.rank])
            )
            node.form_comp_node(1, 1)
            return node
        else:
            node = TraceNode(
                "nccl:param_allgather" + 
                "~iter~" + str(get_iteration()) +
                "~pp_rank~" + str(parallel_state.get_pipeline_model_parallel_rank()) + 
                "~tp_rank~" + str(parallel_state.get_tensor_model_parallel_rank()) + 
                "~model_chunk_id~" + str(model_chunk_id)
            ...
            node.form_comm_coll_node(int(get_param_all_gather_num()), 2, "dp_group")
            return node
    ...
    def __init__(
        ...
        NOTE from here we know that
            all_gather_index:   1   2   3   4   5   ...
            model_chunk_id:     0   0   1   1   2   ...
            param:              0   1   0   1   0   ...
        '''

        self.rank = rank
        # {model_chunk_id: all_gather_handle}
        self.all_gather_handles: Dict[int, Tuple[str, TraceNode]] = {}

        
        self.num_model_chunk = (
            1 if args.virtual_pipeline_model_parallel_size is None 
            else args.virtual_pipeline_model_parallel_size
        )
        for model_chunk_id in range(self.num_model_chunk):
            self.all_gather_handles[model_chunk_id] = None

        # self.overlap_param_gather = self.config.overlap_param_gather
        self.overlap_param_gather = args.overlap_param_gather
        self.remove_pre_hook_handle: bool = True
        self.pre_hook_handle = self._make_forward_pre_hook()
        if self.overlap_param_gather:
            self.enable_pre_hook()

        self.update_successful: bool = False

        self.debug_mode: bool = debug_mode
        self.skip_param_gather: bool = (args.data_parallel_size == 1)
    ...
    def _dispatch_gather_model_params(
        ...
        if self.update_successful:
            ...
            assert model_chunk_id in self.all_gather_handles.keys(), \
                f"{model_chunk_id} not found in rank {self.rank}'s all_gather_handles."
            ...
            grad_ag_node = self.get_param_allgather_node(model_chunk_id)
            if self.debug_mode:
                print(f"rank {self.rank} model_chunk {model_chunk_id} param sync begin.")
            if async_op:
                ctrl_node = trace.get_last_node_name()
                self.all_gather_handles[model_chunk_id] = (ctrl_node, grad_ag_node)
            else:
                if self.debug_mode:
                    print(f"rank {self.rank} model_chunk {model_chunk_id} param sync finish.")
                trace.add_node_with_ctrl_deps(grad_ag_node)
    ...
    def _make_forward_pre_hook(self):
        ...
        def hook(
            ...
            # NOTE we don't use bukect so all param is sync in once
            self._finish_param_sync_helper(model_chunk_id, trace)
    ...
    def finish_param_sync(
        ...
        assert model_chunk_id in self.all_gather_handles.keys(), \
            f"{model_chunk_id} not found in rank {self.rank}'s all_gather_handles."
        if self.all_gather_handles[model_chunk_id] is None:
            return
    ...
    def _finish_param_sync_helper(
        ...
        assert model_chunk_id in self.all_gather_handles.keys(), \
            f"{model_chunk_id} not found in rank {self.rank}'s all_gather_handles."
        all_gather_handle = self.all_gather_handles[model_chunk_id]
        if all_gather_handle is not None:
            # all_gather_handle.wait()
            merge_branch_nodes(
                ctrl_deps=[all_gather_handle[0]],
                branch_nodes=[all_gather_handle[1]],
                trace=trace
            )
            if self.debug_mode:
                print(f"rank {self.rank} model_chunk {model_chunk_id} param sync finish.")
            ...
            # NOTE Every model chunk is in one bucket
            next_model_chunk_id = model_chunk_id + 1
            if next_model_chunk_id < self.num_model_chunk:
                self._dispatch_gather_model_params(
                    model_chunk_id=next_model_chunk_id,
                    trace=trace
                )
    ...
    def _reset_metadata_and_sync_gather_all_model_params(
        self, 
        trace: PytorchPlusTrace, 
        force_sync: bool
    ):
        ...
        for key in self.all_gather_handles.keys():
            self.all_gather_handles[key] = None
    ...
    def step(
        ...
        self.update_successful = super().step()
        ...
        self._reset_metadata_and_sync_gather_all_model_params(trace=trace, force_sync=False)
        # if timers is not None:
