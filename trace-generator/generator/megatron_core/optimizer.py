...
class MixedPrecisionOptimizer(MegatronOptimizer):
    ...
    def step(self):
        ...
        #     )
        # NOTE self._copy_model_grads_to_main_grads()
        # if timers is not None:
        ...
        #             barrier=self.config.barrier_with_L1_time
        #         )
        # NOTE     found_inf_flag = self._unscale_main_grads_and_check_for_nan()
        #     if timers is not None:
        #         timers('optimizer-unscale-and-check-inf').stop()

        #     # We are done with scaling gradients
        #     # so we can update the loss scale.
        # NOTE     self.grad_scaler.update(found_inf_flag)
        ...
        # NOTE    grad_norm = self.clip_grad_norm(self.config.clip_grad)
        # if timers is not None:
        ...
        #         barrier=self.config.barrier_with_L1_time
        #     )
        # NOTE num_zeros_in_grad = self.count_zeros() if self.config.log_num_zeros_in_grad else None
        # if timers is not None:
        ...
        #         barrier=self.config.barrier_with_L1_time
        #     )
        # NOTE self._copy_main_params_to_model_params()
        # if timers is not None:
