...
#     assert args.yaml_cfg is None, "yaml_cfg is not support yet"
...
def parse_json_args(
    ...
    necessary_keys: List[str] = [
        # train args
        "mlp-worker-num", "world-size", "pipeline-model-parallel-size", 
        "context-parallel-size", "tensor-model-parallel-size", 
        "sequence-parallel", "vpp-enable", "num-layers-per-virtual-pipeline-stage", 
        "micro-batch-size", 
        "forward_only", "no-overlap-p2p-communication", "use-distributed-optimizer", 
        "overlap-grad-reduce", "no-delay-grad-reduce", "overlap-param-gather", 
        "delay-param-gather", "train-iters", 
        # model args
        "model-type", "seq-length", 
        "decoder-seq-length", "hidden-size", "ffn-hidden-size", "num-attention-heads", 
        "num-layers", "vocab-size", 
        "swiglu", "untie-embeddings-and-output-weights"
    ]
    for key in necessary_keys:
        assert key in json_args.keys(), \
            f"{key} not found in args."

    args.mlp_worker_num =                       json_args["mlp-worker-num"]
    args.world_size =                           json_args["world-size"]
    args.pipeline_model_parallel_size =         json_args["pipeline-model-parallel-size"]
    args.context_parallel_size =                json_args["context-parallel-size"]
    args.tensor_model_parallel_size =           json_args["tensor-model-parallel-size"]
    args.sequence_parallel =                    (json_args["sequence-parallel"] == 1)
    args.vpp_enable =                           json_args["vpp-enable"]
    args.num_layers_per_virtual_pipeline_stage =    json_args["num-layers-per-virtual-pipeline-stage"]
    args.micro_batch_size =                     json_args["micro-batch-size"]
    args.overlap_p2p_communication =            (json_args["no-overlap-p2p-communication"] == 0)
    args.use_distributed_optimizer =            (json_args["use-distributed-optimizer"] == 1)
    args.overlap_grad_reduce =                  (json_args["overlap-grad-reduce"] == 1)
    args.delay_grad_reduce =                    (json_args["no-delay-grad-reduce"] == 0)
    args.overlap_param_gather =                 (json_args["overlap-param-gather"] == 1)
    args.delay_param_gather =                   (json_args["delay-param-gather"] == 1)
    args.train_iters =                          json_args["train-iters"]
    args.model_type =                           json_args["model-type"]
    args.seq_length =                           json_args["seq-length"]
    args.decoder_seq_length =                   json_args["decoder-seq-length"]
    args.hidden_size =                          json_args["hidden-size"]
    args.ffn_hidden_size =                      json_args["ffn-hidden-size"]
    args.num_attention_heads =                  json_args["num-attention-heads"]
    args.num_layers =                           json_args["num-layers"]
    args.vocab_size =                           json_args["vocab-size"]
    args.swiglu =                               (json_args["swiglu"] == 1)
    args.untie_embeddings_and_output_weights =  (json_args["untie-embeddings-and-output-weights"] == 1)
    ...
    if "make-vocab-size-divisible-by" in json_args.keys():
        args.make_vocab_size_divisible_by = (json_args["make-vocab-size-divisible-by"] == 1)
    else:
        # default setting in megatron
        args.make_vocab_size_divisible_by = 128
    
    if "stable-embedding" in json_args.keys():
        args.stable_embedding = (json_args["stable-embedding"] == 1)
    else:
        args.stable_embedding = False
    
    if "num-query-groups" in json_args.keys():
        args.num_query_groups = json_args["num-query-groups"]
    else:
    ...
    # pre process
    if not args.vpp_enable:
        args.num_layers_per_virtual_pipeline_stage = None
    del args.vpp_enable
    
    if "global-batch-size-mlp-multiplier" in json_args.keys():
        args.global_batch_size = json_args["global-batch-size-mlp-multiplier"] * args.mlp_worker_num
    else:
        assert "global-batch-size" in json_args.keys()
        args.global_batch_size = json_args["global-batch-size"]
...
def validate_args_extern(
    args: Namespace
):  
    # from Megatron-LM/megatron/training/microbatches.py: build_num_microbatches_calculator
    ...
    if args.rampup_batch_size is None:
        ...
        assert args.num_micro_batches >= 1
        # self.current_global_batch_size = global_batch_size
        if args.rank == 0:
            print('> setting number of micro-batches to constant {}'.format(
                args.num_micro_batches), flush=True)
    
    # from Megatron-LM/megatron/training/tokenizer/tokenizer.py: build_tokenizer
    # set padded vocab size if not defined
    ...
    # from Megatron-LM/pretrain_gpt.py: model_provider
    # set share_embeddings_and_output_weights
    args.share_embeddings_and_output_weights = not args.untie_embeddings_and_output_weights
    from generator.overlap_grad_reduce_helper import set_share_embeddings_and_output_weights
    set_share_embeddings_and_output_weights(args.share_embeddings_and_output_weights)

    # custom args for simulation
    from generator.et_generator import (
        get_transformer_flops_per_gpu, set_transformer_flops_per_gpu,
        get_embedding_flops_per_gpu, set_embedding_flops_per_gpu,
        calculate_transformer_flops, calculate_transformer_param_num,
    )
    from generator.overlap_grad_reduce_helper import (
        get_grad_reduce_scatter_num, set_grad_reduce_scatter_num,
        get_embedding_grad_allreduce_num, set_embedding_grad_allreduce_num
    )
    from generator.megatron_core.distrib_optimizer import (
        get_param_all_gather_num, set_param_all_gather_num
    )

    assert get_transformer_flops_per_gpu() is None, "TRANSFORMER_FLOPS_PER_GPU is repeatedly initialized."
    assert get_embedding_flops_per_gpu() is None, "EMBEDDING_FLOPS_PER_GPU is repeatedly initialized."
    (total_flops, embedding_flops) = calculate_transformer_flops(args)
    set_transformer_flops_per_gpu(total_flops/args.world_size)
    set_embedding_flops_per_gpu(embedding_flops/(args.world_size//args.pipeline_model_parallel_size*2))
    
    # calculate param num
    param_in_transform_layer, param_in_embedding_layer = \
        calculate_transformer_param_num(args=args, verbose=True)
    param_num_per_gpu = param_in_transform_layer/(args.pipeline_model_parallel_size*args.tensor_model_parallel_size)
    param_num_per_chunk = param_num_per_gpu if args.virtual_pipeline_model_parallel_size is None \
                            else param_num_per_gpu/args.virtual_pipeline_model_parallel_size
    embedding_allreduce_num = param_in_embedding_layer/args.tensor_model_parallel_size
    assert get_grad_reduce_scatter_num() is None, "GRAD_REDUCE_SCATTER_NUM is repeatedly initialized."
    set_grad_reduce_scatter_num(param_num_per_chunk)
    assert get_param_all_gather_num() is None, "PARAM_ALL_GATHER_NUM is repeatedly initialized."
    set_param_all_gather_num(param_num_per_chunk/args.data_parallel_size)
    assert get_embedding_grad_allreduce_num() is None, "EMBEDDING_GRAD_ALLREDUCE_NUM is repeatedly initialized."
    set_embedding_grad_allreduce_num(embedding_allreduce_num)
