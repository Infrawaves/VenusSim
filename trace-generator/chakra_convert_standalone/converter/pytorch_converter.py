...
from chakra_convert_standalone.schema.protobuf.et_def_pb2 import (
    ALL_GATHER,
    ALL_REDUCE,
    ALL_TO_ALL,
    BROADCAST,
...
from chakra_convert_standalone.schema.protobuf.et_def_pb2 import AttributeProto as ChakraAttr
from chakra_convert_standalone.schema.protobuf.et_def_pb2 import Node as ChakraNode
from chakra_convert_standalone.third_party.utils.protolib import encodeMessage as encode_message
from chakra_convert_standalone.converter.pytorch_node import PyTorchNode, PyTorchNodeType
...
class PyTorchConverter:
    ...
    def convert(self) -> None:
        ...
        root_node_ids = [node.id for node in chakra_nodes.values() if self.is_root_node(node)]

        # print(chakra_nodes[0].name)
        # print(chakra_nodes[1].name)
        # chakra_nodes[1].data_deps.append(chakra_nodes[0].id)
        # print(chakra_nodes[2].name)
        # chakra_nodes[2].data_deps.append(chakra_nodes[0].id)

        # for root_node in root_nodes:
        #     self.convert_ctrl_dep_to_data_dep(pytorch_nodes, chakra_nodes, root_node)
        
        self.convert_ctrl_dep_to_data_dep_new(pytorch_nodes, chakra_nodes, root_node_ids)
        chakra_nodes = self.remove_dangling_nodes(chakra_nodes)

        # for chakra_node in chakra_nodes.values():
        #     print(chakra_node.id)
        #     for data_ctrl_id in chakra_node.data_deps:
        #         print("    ", data_ctrl_id)
        ...
        # for chakra_node in chakra_nodes.values():
        #     print(chakra_node.attr[9], chakra_node.attr[10])
    ...
    def _parse_and_instantiate_nodes(
        ...
        pytorch_root_nids = []
        # we do not use parent because all dependenceis are in ctrl_dep: List[int]
        ...
        for _, pytorch_node in pytorch_nodes.items():
            if (pytorch_node.get_op_type() == PyTorchNodeType.CPU_OP) or (
                ...
                if chakra_node.type == COMM_COLL_NODE:
                    ...
                    # if collective_comm_type != BaseException:
                    # print(pytorch_node.name, collective_comm_type, pytorch_node.comm_size)
                    chakra_node.attr.extend(
                            [
                                ChakraAttr(name="comm_type", int64_val=collective_comm_type),
                                ChakraAttr(name="comm_size", int64_val=pytorch_node.comm_size),
                            ]
                        )
                elif chakra_node.type in {COMM_SEND_NODE, COMM_RECV_NODE}:
                    if not hasattr(pytorch_node, "comm_src"):
                        self.logger.error(f"{pytorch_node.name} does not have attr comm_src", 1)
                    if not hasattr(pytorch_node, "comm_dst"):
                        self.logger.error(f"{pytorch_node.name} does not have attr comm_dst", 1)
                    if not hasattr(pytorch_node, "comm_tag"):
                        self.logger.error(f"{pytorch_node.name} does not have attr comm_tag", 1)
                    chakra_node.attr.extend(
                            [
                                ChakraAttr(name="comm_src", int64_val=pytorch_node.comm_src),
                                ChakraAttr(name="comm_dst", int64_val=pytorch_node.comm_dst),
                                ChakraAttr(name="comm_tag", int64_val=pytorch_node.comm_tag),
                                ChakraAttr(name="comm_size", int64_val=pytorch_node.comm_size),
                            ]
                        )
                chakra_nodes[chakra_node.id] = chakra_node
                # print(chakra_node.attr)
                ...
                #     if hasattr(pytorch_gpu_node, "comm_size"):
                #         print(pytorch_gpu_node.name, pytorch_gpu_node.comm_size)
                #         print(chakra_node.type, COMP_NODE, COMM_COLL_NODE, COMM_SEND_NODE, COMM_RECV_NODE)
    ...
    def convert_to_chakra_node(self, chakra_nodes: Dict[int, ChakraNode], pytorch_node: PyTorchNode) -> ChakraNode:
        ...
        for parent_id in pytorch_node.parent:
            if parent_id in chakra_nodes:
                chakra_node.ctrl_deps.append(parent_id)
        ...
        chakra_node.attr.extend(
            [
                ...
                # ChakraAttr(name="comm_group", string_val=extend_attr_dict["comm_group"]),
                # ChakraAttr(name="num_ops", int32_val=extend_attr_dict["num_ops"]),
                # ChakraAttr(name="tensor_size", int32_val=extend_attr_dict["tensor_size"]),
            ]
        )

        #add new attr
        extend_attr_dict = {"comm_group": "", "num_ops": 0, "tensor_size": 0}
        for extend_attr_key in extend_attr_dict.keys():
            if(hasattr(pytorch_node, extend_attr_key)):
                extend_attr_dict[extend_attr_key] = getattr(pytorch_node, extend_attr_key)
                if type(extend_attr_dict[extend_attr_key]) is str:
                    chakra_node.attr.extend([ChakraAttr(name=extend_attr_key, string_val=extend_attr_dict[extend_attr_key])])
                elif type(extend_attr_dict[extend_attr_key]) is int:
                    chakra_node.attr.extend([ChakraAttr(name=extend_attr_key, int64_val=extend_attr_dict[extend_attr_key])])
                else:
                    exit(1)
        return chakra_node
    ...
    def get_chakra_node_type_from_pytorch_node(self, pytorch_node: PyTorchNode) -> int:
        ...
        if (
            # pytorch_node.is_gpu_op() and 
            (("ncclKernel" in pytorch_node.name or "ncclDevKernel" in pytorch_node.name) or 
            ("c10d::" in pytorch_node.name or "nccl:" in pytorch_node.name))
        ):
    ...
    def convert_ctrl_dep_to_data_dep_new(
        self,
        pytorch_nodes: Dict[int, PyTorchNode],
        chakra_nodes: Dict[int, ChakraNode],
        root_node_ids: List[int], 
    ) -> None:
        if len(pytorch_nodes) != len(chakra_nodes):
            error_msg = "Different length found in convert_ctrl_dep_to_data_dep_new."
            self.logger.error(error_msg, exc_info=1)

        for pytorch_node in pytorch_nodes.values():
            if pytorch_node.id not in chakra_nodes.keys():
                error_msg = f"pytorch node id {pytorch_node.id} not found in chakra nodes."
                self.logger.error(error_msg, exc_info=1)
            if pytorch_node.id in root_node_ids:
                continue
            chakra_node = chakra_nodes.get(pytorch_node.id)
            for ctrl_id in pytorch_node.parent:
                chakra_node.data_deps.append(ctrl_id)
